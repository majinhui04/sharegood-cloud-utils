...
const fireBefore = function (exHooks, config) {
    exHooks.forEach(({ onBefore }) => onBefore && onBefore(config))
}
const fireComplete = function (exHooks, config, isResolve, resOrErr) {
    exHooks.forEach(
        ({ onComplete }) =>
            onComplete && onComplete(config, isResolve, resOrErr),
    )
}
const complete = function (promise, exHooks, config) {
    promise
        .then((res) => fireComplete(exHooks, config, true, res))
        .catch((e) => e)
