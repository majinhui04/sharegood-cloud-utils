...
- 重复请求过滤
- 内部封装上传和下载
- 提供数据请求前以及完成后的多个Hook拦截器
- 支持重试机制
- 对于`application/x-www-form-urlencoded`的请求完成内部处理
- 支持防止 CSRF/XSRF
...
基础参数与axios保持一致（`axios`相关参数请参阅 [http://www.axios-js.com/zh-cn/docs/](http://www.axios-js.com/zh-cn/docs/)）
| 参数      | 说明          | 类型      | 可选值                           | 默认值  |
|---------- |-------------- |---------- |--------------------------------  |-------- |
| baseURL | 接口前缀| String | — | - |
...
| 参数      | 说明          | 类型      | 可选值                           | 默认值  |
|---------- |-------------- |---------- |--------------------------------  |-------- |
| uploadFile | 上传文件 http.uploadFile(url, formData, config)|  | — |  |
| downloadFile | 下载文件 http.downloadFile(url, config)|  | — |  |
...
```javascript
import { createAxios } from '@/sharegood-element-ui/lib/http'
// 返回axios的新实例
...
```javascript
import { createAPI } from '@/sharegood-element-ui/lib/http'
...
// 自定义api拦截器 
API.post('/api/update_password',{newPassword:'123456',repeatPassword:'123456'}).then(data=>{
    console.log(data)
})
// 自定义api函数
